...
// Global variable for hostapd process ID.
pid_t hostapd_pid = -1;
...
// Gets full path of a command (e.g., "/usr/sbin/iw").
...
// Cleanup function: stops hostapd, dnsmasq, and removes the AP interface.
...
// -----------------------
// Terminal UI Functions using ncurses
// -----------------------

// Function to get input from a window; if hide_input is true, input is not
// echoed.
void get_input(WINDOW *win, int starty, int startx, const char *prompt,
               char *input, int maxlen, int hide_input) {
  mvwprintw(win, starty, startx, "%s", prompt);
  wrefresh(win);
  if (hide_input)
    noecho(); // do not echo for password
  else
    echo();
  wgetnstr(win, input, maxlen);
  echo();
}

// Function to display a simple form and collect hotspot credentials.
void get_hotspot_credentials(char *ssid, char *pass) {
  int height = 10, width = 50;
  int starty = (LINES - height) / 2, startx = (COLS - width) / 2;
  WINDOW *win = newwin(height, width, starty, startx);
  box(win, 0, 0);
  mvwprintw(win, 1, 2, "Hotspot Configuration");
  mvwprintw(win, 3, 2, "Enter SSID:");
  mvwprintw(win, 5, 2, "Enter Password:");
  wrefresh(win);

  // Get SSID (normal echo).
  echo();
  mvwgetnstr(win, 3, 16, ssid, 127);
  noecho();

  // Get Password (hidden input).
  get_input(win, 5, 16, "", pass, 127, 1);
...
int main(void) {
  ...
  // -----------------------
  // Terminal UI: Get SSID and Password via ncurses.
  // -----------------------
  char ssid[128] = {0};
  char pass[128] = {0};
  ...
  noecho();
  keypad(stdscr, TRUE);
  get_hotspot_credentials(ssid, pass);
  clear();
  mvprintw(2, 2, "SSID: %s", ssid);
  mvprintw(3, 2, "Password: %s", pass);
  mvprintw(5, 2, "Press any key to continue with hotspot setup...");
  refresh();
  getch();
  endwin();

  // -----------------------
  // Hotspot Setup: Self-checking and configuration.
  // -----------------------
  ...
  // Start dnsmasq for DHCP (with --bind-interfaces to avoid port conflicts).
  char dnsCmd[256];
  ...
  if (!check_dnsmasq_running(dnsmasq_path)) {
    ...
    fprintf(stderr, "Please check for services using port 53 and disable them "
                    "if necessary.\n");
    exit(1);
  } else {
