...
int auto_switch_wifi(const char *nmcli_path) {
    ...
    if (availCount == 0) {
        fprintf(stderr, "No available Wi-Fi networks detected. Auto-switching is not supported on this system.\n");
    ...
    if (system("ping -c 2 google.com >/dev/null 2>&1") != 0) {
        fprintf(stderr, "Connection attempt to \"%s\" did not restore internet connectivity.\n", bestSSID);
        return 1;
    } else {
...
void run_hotspot() {
    ...
    if (freqVal < 5000) {
        fprintf(stderr, "Error: Primary connection is on 2.4 GHz. Please use a 5 GHz network or a separate adapter for the hotspot.\n");
        exit(1);
    }
    ...
    system(ipCmd);
    char linkCmd[128];
    snprintf(linkCmd, sizeof(linkCmd), "sudo %s link set %s up", ip_path, AP_IFACE);
    system(linkCmd);
...
int main() {
    ...
    if (has_colors()) {
        start_color();
        init_pair(1, COLOR_WHITE, COLOR_BLUE);   // For border and title
        init_pair(2, COLOR_YELLOW, COLOR_BLACK);   // For status messages
        init_pair(3, COLOR_GREEN, COLOR_BLACK);    // For menu highlight
    }
    ...
    while (1) {
        ...
        // Draw menu in a grid style (centered)
        ...
        switch (ch) {
            ...
            case 10:  // Enter key
                ...
                if (choice == 0) {       // Start Hotspot
                    ...
                    // If hotspot is still running, ask confirmation to stop it
                    if (hotspot_pid > 0) {
                        clear();
                        box(stdscr, 0, 0);
